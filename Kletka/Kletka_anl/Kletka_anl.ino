...
void tx() {                          // розпізнання команди
 ...
 if(flag_open1==1){
    digitalWrite(LampG, HIGH); //сигнализация о том, что дверь сверху открыта
    digitalWrite(Zamok, LOW);
    delay(4000);
    digitalWrite(Zamok, HIGH);
    digitalWrite(LampG, LOW); //сигнализация о том, что дверь сверху открыта
    flag_open1 = 2;
 }
if (digitalRead(anal_comm_a12) == LOW && digitalRead(anal_comm_a13) == LOW && digitalRead(anal_comm_a14) == HIGH && flag_open2 == 0)
 ...
 if(flag_open2==1){
    ...
    digitalWrite(LampR, HIGH); //сигнализация о том, что дверь сверху открыта
    delay(4000);
    digitalWrite(Zamok2, HIGH);
    digitalWrite(LampR, LOW);
    flag_open2 = 2;
...
void game() {               //основна ігра
  ...
  while(p==0){
    ...
    digitalWrite(Potolok, HIGH);
    digitalWrite(Vibr, HIGH);
    //if (digitalRead(anal_comm_a12) == LOW || digitalRead(anal_comm_a13) == LOW || digitalRead(anal_comm_a14) == LOW){ tx(); }
